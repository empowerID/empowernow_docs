...
- Step 3: Invoke the ARIA MCP Gateway `/mcp/{tool}` and observe PDP constraints
...
- ARIA MCP Gateway (MCP) — `POST /mcp/{tool_id}`
- ARIA Shield — `/chat/completions` (provider‑compatible surface)
