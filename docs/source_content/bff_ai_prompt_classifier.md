...
- policy_export (object, optional): gate PDP export of ML category
  - enabled (bool, default true): export to PDP context when true
  - min_conf (float, optional): minimum score required to export
  - labels_allowlist (string[]): only export when label in this list (empty = all)
- guard (enforcement)
