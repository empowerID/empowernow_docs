...
## 🏗️ **Domain Inheritance Model: Complete Guide**
    ...
    rules:
      - name: "document-actions"
        ...
        actions: 
          ...
          - "checkout"       # ✅ All SharePoint apps get this
          - "checkin"        # ✅ All SharePoint apps get this
      
      - name: "site-actions"
        effect: PERMIT
        resource: "sharepoint:site"
        actions:
          - "create"         # ✅ All SharePoint apps get this
          - "manage"         # ✅ All SharePoint apps get this
          - "configure"      # ✅ All SharePoint apps get this
```

### **➕ Application-Level Extensions**

**App-Specific Additional Actions** (`applications/sharepoint-dev.yaml`):
```yaml
    ...
    rules:
      - name: "development-only-actions"
        ...
        actions:
          ...
          - "profile"        # ➕ Only dev app gets this
      
      - name: "dev-environment-actions"
        effect: PERMIT
        resource: "sharepoint:environment"
        actions:
          - "reset_data"     # ➕ Only dev app gets this
          - "load_fixtures"  # ➕ Only dev app gets this
```

**Admin App Extensions** (`applications/sharepoint-admin.yaml`):
```yaml
# Additional actions ONLY for admin app
version: "1.0" 
policies:
  - name: "sharepoint-admin-extensions"
    rules:
      - name: "admin-only-actions"
        effect: PERMIT
        resource: "sharepoint:site"
        actions:
          - "audit"          # ➕ Only admin app gets this
          - "backup"         # ➕ Only admin app gets this
          - "restore"        # ➕ Only admin app gets this
          - "impersonate"    # ➕ Only admin app gets this
```
...
|----------------|---------------------------|----------------------------|-------------------|
| `sharepoint-dev` | read, write, delete, share, version, checkout, checkin, create, manage, configure | debug, test, simulate, profile, reset_data, load_fixtures | **13 actions** |
| `sharepoint-admin` | read, write, delete, share, version, checkout, checkin, create, manage, configure | audit, backup, restore, impersonate | **14 actions** |
| `sharepoint-prod` | read, write, delete, share, version, checkout, checkin, create, manage, configure | *(none - production security)* | **10 actions** |

### **⚙️ Application Configuration Control**

```python
# Development app gets domain base + dev extensions
ApplicationConfig(
    id="sharepoint-dev",
    domain="sharepoint",                    # ✅ Maps to sharepoint domain
    inherit_domain_policies=True,           # ✅ Gets ALL domain actions/attributes
    policy_directories=[
        "applications/sharepoint-dev"       # ➕ Gets debug, test, simulate, etc.
    ]
)

# Production app gets domain base only (secure by default)
ApplicationConfig(
    id="sharepoint-prod",
    domain="sharepoint",                    # ✅ Maps to sharepoint domain  
    inherit_domain_policies=True,           # ✅ Gets ALL domain actions/attributes
    policy_directories=[]                   # ❌ No additional actions (secure)
)

# Admin app gets domain base + admin extensions
ApplicationConfig(
    id="sharepoint-admin",
    domain="sharepoint",                    # ✅ Maps to sharepoint domain
    inherit_domain_policies=True,           # ✅ Gets ALL domain actions/attributes
    policy_directories=[
        "applications/sharepoint-admin"     # ➕ Gets audit, backup, restore, etc.
    ]
)
```

### **🔄 5-Level Policy Resolution Order**

```mermaid
graph TD
    A["1. Global Policies<br/>(System-wide defaults)"] 
    A --> B["2. Domain Shared Policies<br/>(sharepoint-actions.yaml<br/>sharepoint-attributes.yaml)"]
    B --> C["3. Cross-Environment Policies<br/>(Span dev/test/prod)"]
    C --> D["4. Domain-Environment Policies<br/>(development/ or production/)"]
    D --> E["5. Application-Specific Policies<br/>(sharepoint-dev.yaml overrides)"]
    
    F["Final Result"] --> G["✅ All inherited actions/attributes<br/>✅ Environment-specific rules<br/>✅ App-specific extensions"]
```

**Resolution Logic:**
```python
# When evaluating "sharepoint-dev" request:
effective_policies = (
    global_policies +                    # System-wide defaults (level 1)
    domain_shared_policies +             # sharepoint-actions.yaml (level 2)  
    cross_environment_policies +         # Span environments (level 3)
    domain_environment_policies +        # development/ specific (level 4)
    application_specific_policies        # sharepoint-dev.yaml (level 5)
)
# More specific policies override general ones
# Within same level, last loaded wins
```

### **💡 Real-World Use Cases**

**Use Case 1: Consistent Action Definitions**
```yaml
# Instead of repeating in every app:
# ❌ sharepoint-dev.yaml: actions: ["read", "write", "delete"]  
# ❌ sharepoint-test.yaml: actions: ["read", "write", "delete"]
# ❌ sharepoint-prod.yaml: actions: ["read", "write", "delete"]

# ✅ Define once in domain/sharepoint/shared/actions.yaml
# ✅ All apps automatically inherit: read, write, delete
# ✅ Zero duplication, consistent definitions
```

**Use Case 2: Environment-Specific Security**
```yaml
# Development apps get debug capabilities
sharepoint-dev:
  inherited: [read, write, delete]     # From domain
  extensions: [debug, test, profile]   # From app-specific
  
# Production apps are locked down  
sharepoint-prod:
  inherited: [read, write, delete]     # From domain
  extensions: []                       # No debug capabilities (secure)
```

**Use Case 3: Role-Based Applications**
```yaml
# Regular user app
sharepoint-user:
  inherited: [read, write, share]      # Basic domain actions
  extensions: []                       # No admin capabilities
  
# Admin app gets additional powers
sharepoint-admin:
  inherited: [read, write, share]      # Basic domain actions  
  extensions: [audit, backup, restore, configure]  # Admin-only actions
```  
