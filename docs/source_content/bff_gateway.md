...
Classifier-first summary (LLM gateway)
- For LLM endpoints, the BFF predicts a category from the prompt first and exports it to the PDP (`context.category_*`). The PDP decides Allow/Deny and can attach constraints (models, tokens, egress) and obligations (audit/workflows). In strict mode, the BFF’s guard may still block based on label/threshold.
...
sequenceDiagram
  ...
  BFF->>Classifier: predict
  Classifier-->>BFF: category
  BFF->>PDP: evaluate(context: model, category)
  PDP-->>BFF: decision + constraints
  alt Deny
    BFF-->>Client: 403
  else Allow
    BFF->>Provider: call
...
### Capabilities by scenario (updated)
...
- **Authorization (PDP)**
  ...
  - Classifier-first export: BFF classifies prompts and exports `category_*` to PDP context; PDP decides Allow/Deny and may attach constraints/obligations.
...
- **Observability**
  ...
  - Kafka-structured business logs:
    ...
    - PDP client logs for token fetch, request success/error, cache; obligation dispatch best-effort logs (audit/workflow).
...
### New enhancements (gateway PDP, classifier-first, and obligations)

- Classifier-first export to PDP context (category fields) so policy can decide.
- Privacy modes for prompt journaling (sanitized | balanced | raw) are policy-driven via PDP constraints/obligations; default is sanitized-only with sanitized events to Kafka/CH.
...
- Obligation dispatcher: `audit_log` → Kafka; `run_workflow` → CRUDService `/workflow/start` with correlation and ARNs.
